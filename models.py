...
class Influencers_involved(db.Model):
	...
	posts=db.relationship("Posts_done", cascade="all,delete",backref=db.backref("by_influencer"))   #one-many with posts table #url dena hein
...
class Campaign(db.Model):
	...
	status=db.Column(db.Integer,default=0)    #0=pending, 1=active, 2=completed
	age_req=db.Column(db.Integer)
	platform=db.Column(db.Integer)   #eg. 0,1,3,2    fb,insta,tw,yt
	...
	c_type=db.relationship("Campaign_type", cascade="all,delete",backref=db.backref("from_campaign")) # 1=brand awareness(pan india) 2=event promotion(pan india) 3=viral marketing 4=user generated content text/images 5=app promotion and installs 6=user generated content video 7=brand awareness(city) 8=event awareness(city)
